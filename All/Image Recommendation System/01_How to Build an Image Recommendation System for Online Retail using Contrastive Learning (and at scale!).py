...
def get_percentage_of_data(x_train, x_percentage):
    ...
    indices = np.random.choice(num_samples, size=num_samples_selected, replace=False)
...
# getting only 1 % percentage of data

train = get_percentage_of_data(train,0.1)
test = get_percentage_of_data(test,0.1)
...
def percentage_of_content(main_df,percentage):
    n = int(percentage / 100 * len(main_df))

    # Randomly select 10% of the array indices
    indices = np.random.choice(len(main_df), size=n, replace=False)

    # Select the corresponding elements of the array
    return main_df[indices]

# COMMAND ----------

train = percentage_of_content(train,10)
test = percentage_of_content(test,10)
